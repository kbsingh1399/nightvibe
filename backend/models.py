...
class Event(Base):
    ...
    approved_perks = Column(JSON, default=list) # [{id, name, type}]
    created_at = Column(DateTime, default=datetime.datetime.utcnow)
...
class BookingPass(Base):
    ...
    id = Column(String, primary_key=True, index=True) # e.g. TKT-7782
    event_id = Column(String, ForeignKey("events.id"), nullable=False)
    venue_id = Column(String, ForeignKey("venues.id"), nullable=False)
    promoter_id = Column(String, ForeignKey("promoters.id"), nullable=False)
    ...
    qr_token_secret = Column(String, nullable=False) # Server secret for HMAC
...
class EscrowLedger(Base):
    ...
    gst_18pct = Column(Integer, default=0)
